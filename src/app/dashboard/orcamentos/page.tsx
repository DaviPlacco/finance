  ...
  return (
    <div className="space-y-6 sm:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {selectedCategoryForDetails && (
        <ModalPortal>
          <div 
            className="fixed inset-0 z-[150] w-screen h-screen flex items-center justify-center p-3 sm:p-4 bg-slate-950/70 backdrop-blur-md animate-in fade-in duration-200"
            onClick={() => setSelectedCategoryForDetails(null)}
          >
            <div 
              className="bg-white dark:bg-slate-900 w-full max-w-2xl max-h-[92vh] sm:max-h-[88vh] rounded-2xl sm:rounded-3xl shadow-2xl p-4 sm:p-6 md:p-7 relative border border-slate-200 dark:border-slate-800 flex flex-col animate-in zoom-in-95 duration-200"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Header */}
              <div className="flex items-center justify-between gap-3 pb-3.5 sm:pb-5 border-b border-slate-100 dark:border-slate-800 shrink-0">
                <div className="flex items-center gap-2.5 sm:gap-3.5 min-w-0 flex-1">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span className="px-2 py-0.5 text-[10px] sm:text-xs font-bold rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 shrink-0">
                        {getPeriodLabel()}
                      </span>
                    </div>
                    <p className="text-[11px] sm:text-xs text-slate-500 dark:text-slate-400 mt-0.5 truncate sm:whitespace-normal">
                      Histórico discriminado de despesas associadas
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-1 shrink-0">
                  <button
                    ...
                    className="p-1.5 sm:p-2 text-slate-400 hover:text-primary hover:bg-primary/10 rounded-xl transition-colors"
                  ...
                  <button 
                    onClick={() => setSelectedCategoryForDetails(null)}
                    className="p-1.5 sm:p-2 text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
                    title="Fechar"
                  >
                    <X className="w-5 h-5" />
                  </button>
              ...
              {/* Overview KPI Cards */}
              <div className="grid grid-cols-3 gap-2 sm:gap-3 my-3 sm:my-5 shrink-0">
                <div className="p-2.5 sm:p-3.5 rounded-xl sm:rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/70 dark:border-slate-700/60 flex flex-col justify-between">
                  <span className="text-[9px] sm:text-[11px] font-bold uppercase tracking-wider text-slate-400 truncate">Total Gasto</span>
                  <span className={`text-xs sm:text-lg md:text-xl font-black mt-0.5 sm:mt-1 truncate ${detailsTextColor}`} title={formatCurrency(detailsSpent)}>
                    {formatCurrency(detailsSpent)}
                  </span>
                  <span className="text-[9px] sm:text-[11px] font-medium text-slate-500 mt-0.5 truncate">
                    {detailsPercentage.toFixed(0)}% do limite
                  </span>
                </div>

                <div className="p-2.5 sm:p-3.5 rounded-xl sm:rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/70 dark:border-slate-700/60 flex flex-col justify-between">
                  <span className="text-[9px] sm:text-[11px] font-bold uppercase tracking-wider text-slate-400 truncate">Teto Limite</span>
                  <span className="text-xs sm:text-lg md:text-xl font-black mt-0.5 sm:mt-1 text-slate-900 dark:text-white truncate" title={formatCurrency(detailsLimit)}>
              ...
              {/* Progress bar */}
              <div className="mb-3 sm:mb-4 space-y-1 shrink-0">
                ...
                <div className="h-2 sm:h-2.5 w-full bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                  <div 
              ...
              {/* Scrollable list */}
              <div className="overflow-y-auto flex-1 min-h-[160px] max-h-[46vh] sm:max-h-[38vh] pr-1 space-y-1.5 sm:space-y-2 custom-scrollbar">
                {categoryDetailsTransactions.length === 0 ? (
                  <div className="py-8 sm:py-10 text-center flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-800/40 rounded-2xl border border-dashed border-slate-200 dark:border-slate-700/60">
                    <Receipt className="w-8 h-8 sm:w-10 sm:h-10 text-slate-300 dark:text-slate-600 mb-2" />
                    <p className="font-bold text-slate-700 dark:text-slate-300 text-xs sm:text-sm">Sem gastos registados</p>
                    <p className="text-[11px] sm:text-xs text-slate-400 mt-0.5">Não foi encontrada nenhuma despesa nesta categoria no período selecionado.</p>
                  </div>
                ) : (
                  categoryDetailsTransactions.map((tx: any) => {
                    ...
                    return (
                      <div 
                        key={tx.id}
                        className="p-2.5 sm:p-3.5 rounded-xl bg-slate-50 hover:bg-slate-100/80 dark:bg-slate-800/60 dark:hover:bg-slate-800 border border-slate-200/60 dark:border-slate-700/50 flex items-center justify-between gap-2.5 sm:gap-3 transition-colors"
                      >
                        <div className="flex items-center gap-2.5 sm:gap-3 min-w-0 flex-1">
                          <div className="p-1.5 sm:p-2 rounded-lg bg-rose-500/10 text-rose-500 shrink-0">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="flex items-center gap-1.5 sm:gap-2 mt-0.5 text-[10px] sm:text-xs text-slate-400 flex-wrap">
                              ...
                              {tx.payment_method && (
                                <>
                                  <span>•</span>
                                  <span className="px-1.5 py-0.2 rounded bg-slate-200/60 dark:bg-slate-700/60 text-[10px] sm:text-[11px] font-medium text-slate-600 dark:text-slate-300">
                                    {tx.payment_method}
                                  </span>
                                </>
                              )}
                              {tx.receipt_image && (
                                <>
                                  <span>•</span>
                                  <span className="flex items-center gap-0.5 text-primary text-[10px] sm:text-[11px] font-semibold">
                        ...
                        <div className="flex flex-col items-end shrink-0 pl-2">
                          <span className="font-black text-xs sm:text-base text-rose-600 dark:text-rose-400 whitespace-nowrap">
              ...
              {/* Modal Footer */}
              <div className="mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-2.5 sm:gap-3 shrink-0">
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
                  <button
                    onClick={() => setSelectedCategoryForDetails(null)}
                    className="flex-1 sm:flex-none justify-center px-4 py-2 sm:py-2.5 text-xs sm:text-sm font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-all"
                  >
                    Fechar
                  </button>
                  <button
                    ...
                    className="flex-1 sm:flex-none justify-center flex items-center gap-1.5 px-4 py-2 sm:py-2.5 text-xs sm:text-sm font-bold bg-primary hover:bg-primary/90 text-white rounded-xl shadow-md shadow-primary/25 transition-all active:scale-95 whitespace-nowrap"
                  >
